...
public class BookClient {
  ...
  private static URI uri = UriBuilder.fromUri("http://localhost/chapitre3-service-1.0/rs").port(8080).build();
...
//    response = client.target(uri).path("book").path(id).request().put(Entity.entity(book, MediaType.APPLICATION_XML));
  ...
  private static void all() {
    ...
    URI rootURI = UriBuilder.fromUri("http://localhost/chapitre3-service-1.0/rs/book").port(8080).build();
