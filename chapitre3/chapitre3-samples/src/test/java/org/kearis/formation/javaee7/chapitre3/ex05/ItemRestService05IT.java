...
public class ItemRestService05IT {
  ...
  public void shouldCheckGetBookURIWithParam() {
    Response response = client.target("http://localhost:8282/05/items/books/456").request().get();
    assertEquals(200, response.getStatus());
  }

  @Test
  public void shouldCheckGetBookURIWithWrongParam() {
    Response response = client.target("http://localhost:8282/05/items/books/abc").request().get();
    assertEquals(404, response.getStatus());
  }

  @Test
  public void shouldCheckPostBookURI() {
    Response response = client.target("http://localhost:8282/05/items/books").request().post(Entity.entity(new Book05("The Hitchhiker's Guide to the Galaxy", 12.5F, "Science fiction comedy book", "1-84023-742-2", 354, false), MediaType.APPLICATION_XML));
    assertEquals(201, response.getStatus());
  }

  @Test
  public void shouldCheckDeleteBookURIWithParam() {
    Response response = client.target("http://localhost:8282/05/items/books/123").request().delete();
    assertEquals(204, response.getStatus());
  }

  @Test
  public void shouldCheckDeleteBookURIWithWrongParam() {
    Response response = client.target("http://localhost:8282/05/items/books/abc").request().delete();
